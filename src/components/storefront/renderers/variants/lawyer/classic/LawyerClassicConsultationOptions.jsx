  ...
  return (
    ...
    >
      <div className="w-full max-w-none">
        <div className={isLawyerFirstHome ? 'max-w-none' : 'max-w-3xl'} data-storefront-anim-item="true">
          ...
          <EditableText as="h2" field="content.heading" label="Consultation heading" source={content.heading ? 'persisted' : 'fallback'} className={`mt-3 text-3xl font-semibold uppercase tracking-[-0.02em] sm:text-4xl ${hasCustomText ? 'text-current' : 'text-primary'} ${isLawyerFirstHome ? 'max-w-none lg:whitespace-nowrap' : ''}`}>
            {content.heading || 'Choose how you would like to begin'}
          </EditableText>
          <EditableText as="p" field="content.body" label="Consultation description" source={content.body ? 'persisted' : 'fallback'} className={`mt-4 max-w-2xl text-sm leading-7 ${hasCustomText ? 'text-current opacity-70' : 'text-slate-500'}`}>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <EditableText as="h3" field="content.items" label={`Consultation option ${index + 1} title`} source={hasPersistedItems ? 'persisted' : 'fallback'} collection="items" itemId={item.id} itemIndex={index} itemField="title" className={`text-lg font-semibold leading-tight ${lawyerClassicToneClass(item.text_color, 'text-primary')}`}>
                      {item.title}
                    </EditableText>
                  </div>
                  {item.description ? (
                  ...
                  <button
                    ...
                    data-storefront-label={`Consultation option ${index + 1} button`}
