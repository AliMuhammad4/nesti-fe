...
export function jurisdictionsForCountry(country) {
  if (String(country).toUpperCase() === "US") return US_STATES;
  if (String(country).toUpperCase() === "CA") return CA_PROVINCES;
  return [];
}
...
export function jurisdictionLabel(country, code) {
  const value = String(code || "").toUpperCase();
  if (!value) return "";
  const match = jurisdictionsForCountry(country).find((item) => item.code === value);
  return match?.label || value;
}

export function regionLabel(country, jurisdiction) {
  return [countryLabel(country), jurisdictionLabel(country, jurisdiction)].filter(Boolean).join(" · ");
}
